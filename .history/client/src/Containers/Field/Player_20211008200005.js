...
class Players extends Component {
  ...
  async componentDidMount(state) {
    let userId = JSON.stringify(localStorage.getItem("localID"));
    fetch("/users", {
      method: "POST",
      headers: {
        "Content-type": "application/json",
      },
      body: userId, //JSON.stringify(localStorage.getItem("localID")),
    }).then(
      console.log(
        "sended IDDDDDDDDD",
        JSON.stringify(localStorage.getItem("localID"))
      )
    );

    fetch("/users")
      //   .then((req) => req.json(this.props))
      .then((res) => res.json())
